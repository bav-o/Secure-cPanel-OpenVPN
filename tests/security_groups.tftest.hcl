...
run "cpanel_admin_ports_vpn_only" {
  command = plan
...
run "cpanel_web_ports_public" {
  ...
  assert {
    ...
    error_message = "HTTP should be open to public for hosted websites"
  }

  assert {
    condition     = aws_vpc_security_group_ingress_rule.cpanel_https.cidr_ipv4 == "0.0.0.0/0"
    error_message = "HTTPS should be open to public for hosted websites"
  }
}